...
import { Building2, LayoutPanelLeft, Lock, LogIn, LogOut, Maximize2, Moon, Plus, Sun, UserCircle2, X } from "lucide-react";
...
export function AppSidebar({
  ...
  const handleEdgeTouchMove = (event) => {
    ...
    const nextOffset = Math.max(0, Math.min(SIDEBAR_WIDTH, currentX - touchStartXRef.current));
  ...
  const handleSidebarTouchMove = (event) => {
    ...
    if (sidebarOpen) {
      setDragOffset(Math.min(0, Math.max(-SIDEBAR_WIDTH, delta)));
      return;
    }

    setDragOffset(Math.max(0, Math.min(SIDEBAR_WIDTH, delta)));
  };
  ...
  const translateX = sidebarOpen
    ? Math.min(0, dragOffset)
    : -SIDEBAR_WIDTH + Math.max(0, dragOffset);
  ...
  const sidebarStyle = isMobile
    ? {
      ...
      boxShadow: sidebarOpen ? "0 24px 80px rgba(15, 23, 42, 0.28)" : "none",
    }
    : {
      position: "relative",
      width: 230,
    ...
    <>
      {isMobile && !sidebarOpen && (
        <div
          ...
          style={{ position: "fixed", top: 0, left: 0, bottom: 0, width: 22, zIndex: 1002 }}
        />
      )}
      {isMobile && sidebarOpen && <button onClick={onCloseSidebar} aria-label="Tutup sidebar" style={{ position: "fixed", inset: 0, background: "rgba(15, 23, 42, 0.45)", border: "none", zIndex: 1000, cursor: "pointer" }} />}
      <div onTouchStart={handleSidebarTouchStart} onTouchMove={handleSidebarTouchMove} onTouchEnd={handleTouchEnd} onTouchCancel={resetGesture} style={{ ...sidebarStyle, background: "var(--sidebar-bg)", borderRight: "1px solid var(--surface-bg)", display: "flex", flexDirection: "column", padding: isMobile ? "18px 0 max(18px, env(safe-area-inset-bottom))" : "24px 0", overflowY: "auto", height: isMobile ? "100dvh" : "100%", willChange: isMobile ? "transform" : undefined }}>
        <div style={{ padding: "0 20px 24px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            {isMobile && (
              <button onClick={onCloseSidebar} aria-label="Tutup menu" style={{ width: 34, height: 34, borderRadius: 10, border: "1px solid var(--surface-muted)", background: "var(--panel-elevated)", color: "var(--text-primary)", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", flexShrink: 0 }}>
                <X size={16} />
              </button>
            )}
          </div>
        ...
        {!authUser && (
          <div style={{ margin: "0 12px 16px", background: "var(--surface-bg)", border: "1px solid rgba(99, 102, 241, 0.2)", borderRadius: 12, padding: "12px" }}>
            ...
            <button onClick={handleOpenLogin} style={{ width: "100%", background: "linear-gradient(135deg,#6366f1,#8b5cf6)", border: "none", color: "#fff", borderRadius: 8, padding: "9px", cursor: "pointer", fontWeight: 700, fontSize: 13, fontFamily: "'DM Sans',sans-serif", display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
              <LogIn size={14} /> Masuk
            </button>
          </div>
        )}
        ...
        <div style={{ padding: "0 12px 10px" }}>
          <button onClick={handleOpenBooking} style={{ width: "100%", background: "linear-gradient(135deg,#6366f1,#8b5cf6)", border: "none", color: "#fff", borderRadius: 12, padding: "12px", cursor: "pointer", fontWeight: 700, fontSize: 14, fontFamily: "'Space Grotesk',sans-serif", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
          ...
          {authUser ? (
            <div>
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
                ...
                <button onClick={handleLogout} title="Logout" style={{ background: "var(--danger-bg)", border: "1px solid var(--danger-soft)", borderRadius: 8, width: 30, height: 30, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", color: "var(--danger)", flexShrink: 0 }}>
                  <LogOut size={13} />
                </button>
              </div>
